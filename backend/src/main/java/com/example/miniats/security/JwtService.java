...
public class JwtService {
    ...
    public String createToken(String username, String displayName) {
        ...
        return Jwts.builder().subject(username).claim("displayName", displayName).issuedAt(Date.from(now)).expiration(Date.from(now.plusSeconds(expirationMinutes * 60))).signWith(key).compact();
    }
